...
- TensorRT models/engines & other configs may need to be rebuilt as a result of package version drift between the Docker images and the onboard system installed packages. 

- TBD: script that hunts down, deletes and regenerates TensorRT engine/plan files for Isaac Perceptor. 
...
NB: It may be necessary to regenerate these models in advance of doing any visual navigation or data collection/data fusion for visual navigation. NVBlox uses TensorRT for the backend model processing and as a result, requires that the models be generated by the version of TensorRT installed in the Docker container which may not necessarily be the version installed on the Nova Carter platform itself due to version drift between the installed base on the robot and that of the provisioned containers. 
This has been observed when NVBlox and TensorRT are newer in a container but pointed at mountpoint containing platform-installed packages which are at whatever version was installed on the Nova Carter Jetpack. 
